#!/usr/bin/env python3
"""
NUMBA-ACCELERATED VERSION of gecco_prequential_maxsat_full_eval.py
fully Numba (next-generation uses Numba RNG) with optional parallel scoring.
...
class StreamConfig:
    ...
    concepts: int = 3
    switch_period: int = 6
    revisit_pattern: Tuple[int, ...] = (0, 1, 0, 1)
    stream_mode: str = 'conflict'  # 'original' or 'conflict'
...
def build_concepts(cfg: StreamConfig) -> List[np.ndarray]:
    # Two stream modes:
    #  - 'original': independent random concepts (as in the baseline script)
    #  - 'conflict': maximally conflicting pair where concept 1 is the bitwise complement of concept 0
    if getattr(cfg, "stream_mode", "original") == "conflict":
        c0 = random_assignment_u8(cfg.n_vars)
        c1 = (1 - c0).astype(np.uint8)
        # If user requested >2 concepts, cycle complements deterministically.
        concepts = [c0, c1]
        while len(concepts) < cfg.concepts:
            concepts.append((1 - concepts[-2]).astype(np.uint8))
        return concepts[: cfg.concepts]
...
def make_clause_satisfied_by(concept: np.ndarray, n: int, k: int, stream_mode: str = "original") -> Clause:
    """Generate one k-clause.

    original mode: random signs, then force at least one literal to satisfy the clause under 'concept'.
    conflict mode : choose ALL k literals aligned with 'concept'. Then the clause is SAT by 'concept'
                    and UNSAT by the bitwise complement assignment (maximally conflicting objective).
    """
    vars_ = random.sample(range(1, n + 1), k)

    if stream_mode == "conflict":
        # Align every literal with concept so it becomes false under the complement assignment.
        # If concept[v-1]==1 -> use +v; else use -v.
        return tuple((1 if concept[v - 1] == 1 else -1) * v for v in vars_)
...
def stream_batches(cfg: StreamConfig, steps: int, concepts: List[np.ndarray]):
    for t in range(steps):
        ...
        m_sig = m - m_noise
        batch = [make_clause_satisfied_by(concept, cfg.n_vars, cfg.k, getattr(cfg, 'stream_mode', 'original')) for _ in range(m_sig)]
...
# ----------------------------
# Clause memory for local-search baselines
# ----------------------------
def build_train_lits_with_memory(batch_lits: np.ndarray, mem_lits: np.ndarray | None, mem_batch: int) -> np.ndarray:
    """Augment the current training batch with a random sample of past clauses.

    Prequential *testing* should remain on the current batch; this is only for the update/training phase.
    """
    if mem_batch <= 0 or mem_lits is None or mem_lits.shape[0] == 0:
        return batch_lits
    n = int(min(mem_batch, mem_lits.shape[0]))
    # sample with replacement for speed and stable behavior when mem_lits is small
    idx = np.random.randint(0, mem_lits.shape[0], size=n)
    mem_sample = mem_lits[idx]
    return np.vstack((batch_lits, mem_sample))

def update_clause_memory(mem_lits: np.ndarray | None, new_lits: np.ndarray, mem_size: int) -> np.ndarray | None:
    """Append clauses to the sliding memory buffer (keep the most recent mem_size clauses)."""
    if mem_size <= 0:
        return mem_lits
    if mem_lits is None or mem_lits.size == 0:
        mem_lits = new_lits.copy()
    else:
        mem_lits = np.vstack((mem_lits, new_lits))
    if mem_lits.shape[0] > mem_size:
        mem_lits = mem_lits[-mem_size:]
    return mem_lits
...
def run_one(seed: int, steps: int, epsilon: float,
    ...
    for _, _, batch in stream_batches(stream_cfg, steps, concepts):
        ...
        fits_mem = pop_score_parallel_u8(batch_lits, ga_pop_mem) if numba_parallel else pop_score_serial_u8(batch_lits, ga_pop_mem)
        s_ga_mem += float(int(fits_mem.max()) if fits_mem.size else 0) / B
        ...
        # GA + clause memory UPDATE (timed)
        train_lits_ga = build_train_lits_with_memory(batch_lits, clause_mem_lits, mem_batch)
        t0 = time.perf_counter()
        ga_pop_mem = ga_step_u8(ga_pop_mem, train_lits_ga, ga_cfg, numba_parallel)
        t_ga_mem += time.perf_counter() - t0

        # Update clause-memory buffer with current batch clauses
        clause_mem_lits = update_clause_memory(clause_mem_lits, batch_lits, mem_size)
...
def main():
    ...
    ap.add_argument("--concepts", type=int, default=2, help="number of underlying concepts")
    ...
    ap.add_argument(
        ...
        type=str,
        default="conflict",
        choices=["original", "conflict"],
        help="Synthetic stream mode. 'conflict' makes concept 1 the complement of concept 0 and generates clauses that are SAT for the active concept but UNSAT for its complement (maximally conflicting drift).",
    )
    # WalkSAT parameters
    ...
    # Clause-memory baselines (augment update batches with past clauses)
    ap.add_argument("--mem_size", type=int, default=2000, help="sliding clause-memory size (number of past clauses); 0 disables")
    ap.add_argument("--mem_batch", type=int, default=200, help="number of memory clauses sampled per update for *_mem baselines")
    ...
    if args.mem_size < 0:
        raise SystemExit("ERROR: --mem_size must be >= 0")
    if args.mem_batch < 0:
        raise SystemExit("ERROR: --mem_batch must be >= 0")
    ...
    for i in range(args.runs):
        seed = args.seed0 + i
        rr = run_one(seed, args.steps, args.epsilon, stream_cfg, ws_cfg, ps_cfg, ga_cfg, args.mem_size, args.mem_batch, args.numba_parallel)
        print(
            f"RUN {i:02d}  seed={seed}  "
            f"WS_warm={rr.mean_ws_warm:.4f}  "
            f"WS_restart={rr.mean_ws_restart:.4f}  "
            f"WS_eps={rr.mean_ws_eps:.4f}  " f"PS_warm={rr.mean_ps_warm:.4f}  "
            f"PS_restart={rr.mean_ps_restart:.4f}  "
            f"PS_eps={rr.mean_ps_eps:.4f}  " f"GA={rr.mean_ga:.4f}  "f"GA_mem={rr.mean_ga_mem:.4f}  "
        ...
        ga_mem_means.append(rr.mean_ga_mem)
        ...
        ga_mem_times.append(rr.t_ga_mem)
    ...
    print(fmt_stats("GA + clause memory", ga_mem_means))
    ...
    print(fmt_time("GA + clause memory", ga_mem_times))
    ...
    for name, baseline in [
        ...
        ("GA_mem vs GA", ga_means),
        ("GA_mem vs WalkSAT warm-start", ws_warm_means),
        ("GA_mem vs ProbSAT warm-start", ps_warm_means),
    ]:
        xs = ga_mem_means if name.startswith("GA_mem") else ga_means
        res = wilcoxon_signed_rank_pvalue(xs, baseline)
